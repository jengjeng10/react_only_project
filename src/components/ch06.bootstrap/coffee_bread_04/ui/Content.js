...
function App({ contents, onClickToContent }) {
    ...
    /* 테이블 특정 행(row)의 셸(cell) 1개를 클릭했습니다. */
            ...
            <tbody>
                {contents.map((item, index) => (
                    <tr id={item.id} key={index}>
                        <td align="left" onClick={ClickItem}>{item.name}</td>
                        {/* 숫자 형식으로 바꾼 다음, 3자리 마다 콤마 유형을 추가합니다. */}
                        <td align="right" onClick={ClickItem}>{Number(item.price).toLocaleString()}</td>
                        <td align="left" onClick={ClickItem}>{item.category === 'bread' ? '빵' : '음료수'}</td>
                    </tr>
                ))}
            </tbody>
        );
