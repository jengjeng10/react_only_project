...
function App({ mode, product, onSubmitInsert, onSubmitUpdate }) {
    ...
    switch (mode) {
        case 'detail':
            return <Display product={product} />; //특정 상품 상세 보기

        case 'get_insert':
            return <CreateContent onSubmitInsert={onSwitchInsert} />; // 상품 등록 화면으로 이동
        case 'get_update':
            /* product는 이전에 사용자가 입력했던 데이터로써, 수정하고자 하는 항목입니다. */
            return <UpdateContent product={product} onSubmitUpdate={onSwitchUpdate} />; // 상품 수정 화면으로 이동
